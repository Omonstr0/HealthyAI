...
app = Flask(__name__)
app.secret_key = 'super_secret_key'  # À sécuriser dans un vrai projet
...
# Clés API Edamam (remplace par les tiennes)
...
# Vérifie les extensions de fichiers autorisées
def allowed_file(filename):
    return '.' in filename and \
           filename.rsplit('.', 1)[1].lower() in ALLOWED_EXTENSIONS

# Fonction d'appel à l'API Edamam
def get_nutrition_from_food(food_query):
    ...
    # Étape 2 : Envoyer vers /nutrients
    payload = {
        "ingredients": [
            {
                "quantity": 1,
    ...
    nutrition_data["label"] = label  # ajouter le nom de l'aliment détecté
...
def forgot_password():
    if request.method == 'POST':
        ...
        if user:
            ...
            # Simuler l'envoi d'email (pour l'instant on affiche dans la console)
...
def reset_password(token):
    ...
    if request.method == 'POST':
        ...
        user.reset_token = None  # Supprimer le token
        db.session.commit()
...
def upload():
    ...
    if allowed_file(file.filename):
        ...
        return render_template('dashboard.html', image_url=filepath, uploads=uploads, nutrition=nutrition, detected_dish=label_formatted)
...
def analyse(upload_id):
    ...
    session['last_nutrition'] = nutrition
    session['last_image'] = os.path.join(app.config['UPLOAD_FOLDER'], upload.filename)
